...
export default function StatsPlots({ earthquakesPerYear, avgMagnitudePerYear }) {
  const processYearlyData = () => ({
    xAxis: {
      type: "category",
      data: earthquakesPerYear.map((item) => item.Year.toString()),
      name: "Year",
      nameLocation: "middle",
      nameGap: 30,
    },
    ...
    series: [
      {
        data: earthquakesPerYear.map((item) => item.count),
        type: "bar",
        color: "#3b82f6",
      },
    ],
  ...
  const processMagnitudeData = () => ({
    xAxis: {
      type: "category",
      data: avgMagnitudePerYear.map((item) => item.Year.toString()),
      name: "Year",
      nameLocation: "middle",
      nameGap: 30,
    },
    ...
    series: [
      {
        data: avgMagnitudePerYear.map((item) => Number(item.averageMagnitude.toFixed(2))),
        type: "line",
        color: "#9333ea",
      },
    ],
