...
export default function EarthquakeMap({ earthquakeData }) {
  ...
  useEffect(() => {
    async function fetchPlateData() {
      ...
      setPlateBoundaries(await bRes.json());
      setPlateNames(await nRes.json());
      setLoadingPlates(false);
    }
    fetchPlateData();
  }, []);

  const stillLoading = loadingPlates || !mapLoaded;
    ...
    <div className="w-[95%] h-[700px] mx-auto overflow-hidden border border-gray-300 shadow-lg relative">
      {stillLoading && (
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white p-4 rounded-lg shadow-md text-lg font-bold text-gray-800 z-[1000]">
          {/* <p>
            {loadingPlates && "GeoJSON-Daten werden geladen..."}
            {!loadingPlates && !mapLoaded && "Kartenkacheln werden gerendert..."}
          </p> */}
          <p>
            {loadingPlates
              ? "Loading GeoJSON plate data..."
              : "Rendering map tiles..."}
          </p>
        </div>
      )}
      <MapContainer
        ...
        // className={`h-full w-full ${stillLoading ? "opacity-50" : "opacity-100"}`}
      ...
      >
        ...
        {plateNames &&
          plateNames.features.map((plate, idx) => {
            const coords = plate.geometry.coordinates[0];
            ...
            return (
              ...
              >
                <span className="font-bold text-black bg-white p-1">
